...
> **核心原则：脚本数字做判断，AI 感觉不做判断。**

AI 处理大量重复任务时会"偷懒"——做几页就宣布完成，实际漏了一大半。以下机制强制 AI 完成全部工作。

### 规则

1. **批量校验/修复任务必须用脚本**，不能靠 AI 逐个判断
2. **脚本退出码是唯一完成标准**：0 = 通过，非 0 = 未完成
3. **禁止 AI 自行宣布"差不多了"**——必须等脚本返回 0
4. **修复后必须重新跑脚本验证**——不能跳过
5. **扫描必须从 .pen 文件实时读取**，不能用缓存数据

### 批量校验/修复 SOP

当用户要求"全量校验"、"全部替换"、"逐页检查"等批量任务时，执行以下流程：

```
第一步：全量拉取
  → mcp_pencil_batch_get(readDepth:5) 获取全部页面
  → 保存到 .pen 文件

第二步：脚本扫描
  → 运行 assets/scan.cjs PRD/xxx.pen
  → 输出精确数字（如 "Hardcoded found: 3635"）
  → 退出码 1 = 有问题，0 = 通过

第三步：批量修复
  → 方案 A（推荐）：直接修改 .pen JSON 文件（脚本读→改→写）
  → 方案 B：用 MCP batch_design 逐批修复（适合小量）
  → ⚠️ 修复前必须确保目标变量已存在于 .pen 文件中
  → ⚠️ 只替换有对应变量的色值，无变量的先创建变量再替换
...
| ❌ 错误 | ✅ 正确 |
|---------|---------|
| AI 做了 3 页就宣布完成 | 脚本扫描全量，输出精确数字 |
| 扫描用缓存 JSON | 每次从 .pen 文件实时读取 |
| 替换色值时不检查变量是否存在 | 先创建缺失变量，再替换 |
| 修复后不验证 | 脚本退出码 = 0 才算完成 |
| MCP set_variables 不验证写入 | 直接修改 .pen JSON 文件的 variables 字段 |

### 脚本文件

| 文件 | 用途 |
|------|------|
| `assets/scan.cjs` | 全量扫描硬编码色值和字体 |
| `assets/fix5.cjs` | 批量修复（直接改 .pen JSON） |
| `assets/check-colors.mjs` | ESM 版扫描（备用） |
| `assets/VERIFY-SOP.md` | 完整 SOP 文档 |
